...
from .utils.parsers import analyze_form_fields, contains_captcha, extract_token
...
class Attack:
    ...
    def _get_login_page(self):
        ...
        # analyze_form_fields(resp.text) # 可以选择性地分析表单
        return resp
    ...
    def _extract_token(self, resp) -> Optional[str]:
        ...
        if not token:
            logging.warning(f"未能提取CSRF Token（如目标无CSRF token可忽略）")
        return token
