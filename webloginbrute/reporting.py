...
class StatsManager:
    ...
    def print_final_report(self):
        ...
        with self.lock:
            ...
            # 避免除以零
            attempts_per_second = self.stats['total_attempts'] / duration if duration > 0 else 0
            ...
            print(f"   - 平均速率: {attempts_per_second:.2f} 次/秒")
            print("-" * 50)
            print(f" 结果分析")
            ...
            peak_mem = self.performance['peak_memory_usage']
            print(f"   - 峰值内存使用: {peak_mem:.2f} MB" if peak_mem > 0 else "   - 峰值内存使用: N/A (psutil未安装)")
            print("="*50)
