        ...
        for file_path in files_to_check:
            try:
                ...
                if file_size > max_size_bytes:
                    raise ConfigurationError(
                        f"文件过大: {file_path} ({file_size / 1024 / 1024:.1f}MB > {self.max_file_size}MB)"
                    )
    ...
    def _check_system_resources(self):
        ...
        try:
            ...
            if available_memory_mb < self.max_memory_mb:
                logging.warning(
                    f"系统可用内存 ({available_memory_mb:.0f}MB) 低于配置的最大内存 ({self.max_memory_mb}MB)"
                )

            # 检查CPU负载
            cpu_percent = psutil.cpu_percent(interval=1)
            if cpu_percent > 90:
                logging.warning(f"系统CPU负载过高: {cpu_percent}%")

        except ImportError:
            logging.debug("psutil未安装，跳过系统资源检查")
        except Exception as e:
            logging.warning(f"检查系统资源时出错: {e}")
