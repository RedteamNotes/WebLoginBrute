...
class Config(BaseModel):
    ...
    def parse_args():
        ...
        parser.add_argument(
            "--csrf", required=False, default=None, help="CSRF token字段名"
        )
        ...
        parser.add_argument(
            "--timeout", type=int, default=30, help="请求超时时间（秒）"
        )
        ...
        parser.add_argument(
            "--progress-file", default="bruteforce_progress.json", help="进度文件路径"
        )
        parser.add_argument(
            "--aggression-level",
            choices=["A0", "A1", "A2", "A3"],
