...
class PerformanceMonitor:
    ...
    def _check_thresholds(self, metrics: PerformanceMetrics):
        ...
        if metrics.memory_percent > self.thresholds['memory_critical']:
            alerts.append(
                ('critical', 'memory', f"内存使用率过高: {metrics.memory_percent:.1f}%"))
        elif metrics.memory_percent > self.thresholds['memory_warning']:
            alerts.append(
                ('warning', 'memory', f"内存使用率较高: {metrics.memory_percent:.1f}%"))

        # 磁盘I/O阈值检查
        if metrics.disk_io_read_mb + metrics.disk_io_write_mb > self.thresholds['disk_io_warning']:
            alerts.append(
                ('warning', 'disk_io', f"磁盘I/O较高: {metrics.disk_io_read_mb + metrics.disk_io_write_mb:.1f}MB/s"))

        # 网络I/O阈值检查
        if metrics.network_sent_mb + metrics.network_recv_mb > self.thresholds['network_warning']:
            alerts.append(
                ('warning', 'network', f"网络I/O较高: {metrics.network_sent_mb + metrics.network_recv_mb:.1f}MB/s"))
    ...
    def print_summary(self):
        ...
        if current_metrics:
            ...
            print(
                f"  内存使用率: {current_metrics.memory_percent:.1f}% ({current_metrics.memory_mb:.1f}MB)")
        ...
        if system_stats:
            ...
            print(
                f"  内存平均使用率: {system_stats.get('memory', {}).get('average_percent', 0):.1f}%")
        ...
        print("  - 网络性能:")
        if current_metrics:
            print(f"    - 发送数据: {current_metrics.network_sent_mb:.1f}MB")
            print(f"    - 接收数据: {current_metrics.network_recv_mb:.1f}MB")
        else:
            print(f"    - 无网络性能数据")

        print("  - 错误统计:")
        if operation_stats.get('failed_operations', 0) > 0:
            print(f"    - 失败操作数: {operation_stats['failed_operations']}")
        else:
            print(f"    - 无错误数据")
