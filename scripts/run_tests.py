...
def run_coverage():
    ...
    if result.returncode == 0:
        ...
        report_result = subprocess.run(
            report_cmd, capture_output=True, text=True, cwd=str(project_root)
        )
...
def run_security_check():
    ...
    # 运行bandit安全检查
    cmd = [sys.executable, "-m", "bandit", "-r", "webloginbrute", "-f", "txt"]
    result = subprocess.run(cmd, capture_output=True, text=True, cwd=str(project_root))
...
def main():
    ...
    # 返回总体结果
    overall_success = (
        tests_passed and coverage_passed and linting_passed and security_passed
    )
